...
- 利用者として、AI がどのプロバイダ・モデルで何を返したかを `/logs` の AI タブ（S-61）で
  後から確認したい（AI 駆動開発の可視化。§1「AI ログ機能（FR-14）で開発プロセス自体を可視化」）。
  この画面は開発者向けの位置づけで、`/settings` の「開発者向け」からのみ到達する
  （requirements v0.1.27）。
...
本 spec の範囲では無し（記録側だけを扱う）。`GET /logs/ai` と `/logs` 画面の接続は
#64 / #187 で完了しており、設計は [`docs/specs/logs-api.md`](logs-api.md) が持つ。
全画面から開くスライドインパネル（P-01）は廃止し、AI ログは S-61（`/logs?tab=ai`）の行を
開いて見る（requirements v0.1.27 / ui-screens v0.10）。画面を経由せずに見るなら
ローカルは SQL / Drizzle Studio、本番は Vercel の関数ログ。
...
| v0.1.3 | 2026-08-28 | §0 / §3 を現状に更新。`GET /logs/ai` と `/logs` 画面の接続は #64 / #187 で完了済み（設計は `logs-api.md`）。AI ログのスライドインパネル（P-01）は廃止され、S-61（`/logs?tab=ai`）の行展開で見る。`/logs` は `/settings` の「開発者向け」からのみ到達する（requirements v0.1.27 / ui-screens v0.10） |
