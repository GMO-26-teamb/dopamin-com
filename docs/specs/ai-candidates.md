...
FR-13（サブドメイン設計）も同じ「生スキーマ + 項目ごと再検証」を採るが、**集合としての制約**
（`www` 必須 / 3〜8 件）だけは救済せず 503 に倒す。理由は
[`subdomain-plan.md`](subdomain-plan.md) §2.5（#167）。
...
| v0.4.2 | 2026-08-28 | §2.1 に FR-13 との対応関係を 1 行追記（#167）。サブドメイン設計も同じ「生スキーマ + 項目ごと再検証」を採るが、集合としての制約（`www` 必須 / 3〜8 件）は救済せず 503 に倒す。理由は `subdomain-plan.md` §2.5 |
