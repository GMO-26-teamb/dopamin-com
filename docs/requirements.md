...
| `/logs` | ログ（開発者向け） | 操作ログ / AI ログ タブ。主ナビには出さず `/settings` の「開発者向け」から入る | FR-14/15 |
| `/settings` | 設定 | パスキー管理、AI 設定、デモリセット | FR-01/16/17 |
| 共通 | サイドバー | 左: 主要 CTA「+ ドメインを取得」+ ナビ（ダッシュボード / 移管 / 設定）。AI ログのスライドインパネルは持たない（AI ログは `/logs` のタブで見る） | — |
...
- 768px（Tailwind の `md`。`--breakpoint-*` は上書きせず既定値のまま）以上: サイドバー常時表示。未満: サイドバーを隠し、Logo + 横スクロールするナビ行 + ログアウト / 「ドメインを取得」を並べた sticky ヘッダ（`MobileNav`）に切り替える（開閉するハンバーガードロワーは持たない）。CTA のラベルはデスクトップと揃える。
...
| `GITHUB_TOKEN` | 公開リポ取得のレート制限緩和（読み取りのみのスコープ）。**`GITHUB_MODE=real` では実質必須**。未設定でも公開リポは読めるが未認証は 60 req/h で、1 提案あたり最大 6 リクエスト投げるため 1 時間に約 10 提案で `RATE_LIMITED`（429）に達し、以降の提案生成が全滅する（未設定のまま `real` にすると初回呼び出しで警告ログが出る。#168） |
...
| 18 | Poll 消化中に**所有者自身のドメイン**へ行う呼び出し（移管 IN 確定時の `importApproved` の `info` など）の `operation_logs.user_id`。現状は §9.1 の文面どおり Poll 由来を一律 NULL にしている（#250）。所有者の `user_id` を付ければ本人が移管完了を自分の操作ログで追えるが（自分のドメインなので NFR-04 の漏洩にはならない）、§9.1「Poll 由来などシステム起点の呼び出しは NULL」に例外を足す要件変更になる。決めるまでは一律 NULL のまま | チーム | 未定 |
...
| v0.1.29 | 2026-08-28 | §17: `GITHUB_TOKEN` を「`GITHUB_MODE=real` では任意」から**実質必須**に改めた（#168）。未認証の GitHub REST は 60 req/h で、FR-13 の 1 提案が最大 6 リクエスト投げるため 1 時間に約 10 提案で `RATE_LIMITED`（429）に達し、`getJson` がレート制限だけは任意の呼び出しでも投げ直すため以降の提案生成が全滅する。要件本文（§7「サーバー側トークン」）が元から認証前提だったので、そちらに合わせた形。実装は例外を投げず（トークン無しでも公開リポは読める）、`GITHUB_MODE=real` かつ未設定のときに初回呼び出しで構造化ログの警告を出す。あわせて 1 提案あたりのリクエスト数を最大 10 → 6 に減らした（ルート直下 + 構造ディレクトリの `contents` 最大 5 回を再帰ツリー 1 回に置き換え。`docs/specs/subdomain-plan.md` §2.2）|
| v0.1.30 | 2026-08-28 | §21.2 に **#18** を追加（#250）。Poll 消化のレジストリ呼び出しが「消化を起動したユーザー」の `user_id` で `operation_logs` に入り、他ユーザーのドメイン名がその起動者の `GET /logs/operations` に出ていた（§9.1「Poll 由来などシステム起点の呼び出しは NULL」/ NFR-04 違反）。文書化済みの挙動へ戻す修正（`consumePoll` を `user_id = null` のコンテキストで走らせる）なので要件本文は変えていない。Poll 消化中に所有者自身のドメインへ行う呼び出しを所有者の `user_id` で記録すべきか（§9.1 に例外を足すか）だけが未決のため #18 に積んだ |
